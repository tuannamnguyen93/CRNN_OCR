...
image = tf.placeholder(tf.float32, shape=(None,581,32, 1), name='img_data')
...
train_length=np.array([30]*cfg.BATCH_SIZE,dtype=np.int32)
...
def encoder_net(_image, scope,is_training,reuse=None):
    with tf.variable_scope(scope, reuse=reuse):
        ...
        enc_outputs, enc_state = tf.nn.bidirectional_dynamic_rnn(cell_fw=cell,cell_bw=cell,inputs=cnn_out,dtype=tf.float32)
...
def build_network(is_training):
    ...
    output_embed = layers.embed_sequence(train_output, vocab_size=cfg.VOCAB_SIZE, embed_dim=cfg.VOCAB_SIZE, scope='embed')
    embeddings = tf.Variable(tf.truncated_normal(shape=[cfg.VOCAB_SIZE, cfg.VOCAB_SIZE], stddev=0.1), name='decoder_embedding')
