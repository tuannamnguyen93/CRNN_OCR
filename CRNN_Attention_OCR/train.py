...
val_img,val_label=cfg.read_data(config.val_dir,'Synthetic Chinese String Dataset_2/test.txt')
...
for cur_epoch in range(cfg.EPOCH):
    ...
    for cur_batch in range(num_batches_per_epoch):
        ...
        if cur_batch%cfg.DISPLAY_STEPS==0:
            summary_loss, loss_result = sess.run([summary_op, loss],feed_dict={image: batch_inputs,train_output: batch_target_in,target_output: batch_target_out, sample_rate: np.min([1., 1.])})
